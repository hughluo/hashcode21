...
def read_input(filename):
    ...
    intersection_to_streets_map = defaultdict(
        set)  # k: intersection, v: set of street
    # k: street, v: amount of cars that have this street in his route
    street_to_traffic_map = defaultdict(int)
...
def output_weighted_street_traffic(f, street_to_traffic_map, intersection_to_streets_map):
    with open(f + ".output", "w") as f:
        ...
        for i in intersection_to_streets_map:
            ...
            intersection_traffic = sum(
                (street_to_traffic_map[t] for t in intersection_to_streets_map[i]))

            for steet in intersection_to_streets_map[i]:
                green_sec_base = 5
                street_traffic_precentage = street_to_traffic_map[steet] / \
                    intersection_traffic
                green_sec = street_traffic_precentage * green_sec_base
...
def drop_streets(intersection_to_streets_map, street_to_traffic_map, threshhold):
    ...
    new_street_to_traffic_map = {k: v for k, v in street_to_traffic_map.items(
    ) if k in available_streets and street_to_traffic_map[k] >= threshhold}
...
if __name__ == "__main__":
    for f in ['a', 'b', 'c', 'd', 'e', 'f']:
        D, I, S, V, F, intersection_to_streets_map, street_to_traffic_map = read_input(
            'input/' + f + '.txt')
        intersection_to_streets_map_with_drop = drop_intersections(
            intersection_to_streets_map, 1)
        new_intersection_to_streets_map, new_street_to_traffic_map = drop_streets(
            intersection_to_streets_map_with_drop, street_to_traffic_map, 1)
        output_weighted_street_traffic(
            f, new_street_to_traffic_map, new_intersection_to_streets_map)
